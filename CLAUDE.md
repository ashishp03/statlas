...
**Current phase:** Foundations-first learning mode (see `learning/roadmap.md`, Stage 3 — transformers). Product build is paused; the p0 rule-based scaffold is the baseline and ship target for when building resumes.
...
### 2026-06-13 — EDA on real data + pivot to foundations-first
**Built:** Verified `nba_api` (free, no key) as the data source and confirmed the three v1 questions (Brunson avg, league PPG leader, playoffs rebound leader) are answerable from `LeagueDashPlayerStats` (PerGame, Regular Season + Playoffs). Ran a full EDA → `learning/days/day5/eda/`: report + 6 figures + cached CSVs. Findings: 582 players × 67 cols, zero nulls / zero duplicate players; coverage floor 1996-97; traded players pre-aggregated to one row (`TEAM_COUNT`); the key reliability rule that leaderboards need a min-games qualifier. Then, at the owner's direction, switched to **foundations-first** learning: authored `learning/roadmap.md` (ground-up concept stack NN→NLP→transformers→LLMs→prompting→text-to-SQL→SQL→RAG→eval, each with example-coding exercises) and parked `learning/curriculum.md`. Updated `progress.md` status + daily log and `days/day5/summary.md`.
**Notes:** Git commits to the `learning` branch are currently blocked by stale `.git/index.lock` + `.git/HEAD.lock` (the sandbox mount can't unlink in `.git`); manual fix: `rm -f .git/index.lock .git/HEAD.lock && git add learning/ && git commit`.
